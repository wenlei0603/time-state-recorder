    ...
    pub fn build_chat_completions_request(
        ...
        let mut body = serde_json::json!({
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You write a neutral Chinese daily desktop-work brief and structured work-overview diary dashboard from structured activity metrics and five-hour reports. Return compact JSON only."
                },
                {
                    "role": "user",
...
fn daily_brief_prompt(
    ...
    format!(
        "Write a neutral daily brief in Chinese. All time fields are {} local time. Return JSON only with keys dailySummaryText, actionTrajectory, comparisonExplanation, diaryDashboard. Do not include advice, praise, criticism, ranking, or value judgment. Avoid words equivalent to productive, wasted, efficient, inefficient, good, bad, should. actionTrajectory must be complete, human-readable, structured around parallel projects, time allocation, workflow pattern, work mode, design/tooling activity, and important evidence, and must not emit UTC, Z, or +00:00 time labels; do not create a raw 5-minute log. diaryDashboard must be a human-readable work overview object that ignores low-level detail and captures collaborators, locations, workTypes, roles, roleHeterogeneity, teamCount, teamDistribution in the teams array, duration distribution in timeDistribution, evidenceSummary, and uncertainty. Treat the teams array as coarse work domains, not raw project hints or every title fragment; merge semantically similar hints into a small set of orthogonal domains. Treat roles as contribution modes such as build, research, write, coordinate, plan, operate, learn, or personal-away; roles must not duplicate workTypes. Treat workTypes as activity categories only. Each distribution row should include a label, short description or summary, activeSeconds when inferable, share when inferable, and evidence. Use uncertainty when evidence is incomplete. date={}, localPeriodStart={}, localPeriodEnd={}, descriptiveStats={}, hourlyMetrics={}, comparison={}, fiveHourReports={}",
        human_report_timezone_label(),
        date,
        minimax_prompt_timestamp(period_start),
        minimax_prompt_timestamp(period_end),
